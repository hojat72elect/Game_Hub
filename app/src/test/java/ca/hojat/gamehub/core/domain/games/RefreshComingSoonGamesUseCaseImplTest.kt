...
class RefreshComingSoonGamesUseCaseImplTest {
    ...
    fun `Emits remote games when refresh is possible`() {
        runTest {
            ...
            coEvery { gamesRemoteDataSource.getComingSoonGames(any()) } returns Ok(DOMAIN_GAMES)
    ...
    fun `Does not save remote games into local data store when refresh is unsuccessful`() {
        runTest {
            coEvery { throttler.canRefreshGames(any()) } returns false
            coEvery { gamesRemoteDataSource.getComingSoonGames(any()) } returns Err(
